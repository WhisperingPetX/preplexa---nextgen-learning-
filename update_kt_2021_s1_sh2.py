...
with open('solutions_2021_s1_sh2.txt', 'r', encoding='utf-8') as f:
    sol_text = f.read()
...
kt_file = 'app/src/main/java/com/example/data/repository/JeeMain2021Session1Shift2QuestionsRepository.kt'
with open(kt_file, 'r', encoding='utf-8') as f:
    kt_content = f.read()

def replace_explanation(match):
    block = match.group(0)
    id_match = re.search(r'id = "\$\{testId\}_s1_sh2_q(\d+)_(phys|chem|math)"', block)
    if not id_match:
        return block
...
print("Updated explanations successfully.")
